...
/* === Preload controls === */
const EAGER_FIRST = 12;            // จำนวนรูปแรกของทั้งอัลบั้มที่โหลด eager/high
const PREFETCH_ROWS_AHEAD = 2;     // โหลดล่วงหน้าอีกกี่ "แถว" ผ่าน IntersectionObserver
const LB_PRELOAD = 2;              // โหลดล่วงหน้ารูปข้างเคียงในไลท์บ็อกซ์ ต่อข้าง
...
function preloadLB(centerIdx, dataCtx) {
  ...
  for (let d = 1; d <= LB_PRELOAD; d++) {
    ...
    [next, prev].forEach((k) => {
      const u = getViewUrl(dataCtx, k);   // 2048
      const img = new Image();
      img.decoding = 'async';
      img.loading = 'eager';
