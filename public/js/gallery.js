...
async function fetchCollections(page = 1, perPage = itemsPerPage) {
  const url = new URL(GALLERY_API, location.origin);
...
function renderGallery(galleries, page, total, totalPagesServer) {
  ...
  const cards = galleries.map((g, i) => {
    const ar = g.aspectRatio || '4/3';
    const cover = g.cover;

    const srcset = buildWebpSrcsetFromCover(cover, [320]);
    ...
    return `
      <article class="masonry-item">
        <a href="${viewUrl}"
           class="group block rounded-lg overflow-hidden transition-all transform hover:-translate-y-1 hover:shadow-xl bg-white shadow-md">
          <div class="relative overflow-hidden" style="aspect-ratio:${ar}">
            <picture>
              ...
              <img
                ...
                alt="${g.title}"
                width="${g.w ?? 320}"
                height="${g.h ?? Math.round((g.w ?? 320) / (Number(ar.split('/')[0]) / Number(ar.split('/')[1] || 1)))}"
