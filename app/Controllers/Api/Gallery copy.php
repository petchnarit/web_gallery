...
class Gallery extends ResourceController
{
    ...
    protected string $publicCacheDir = '/volume1/web_gallery/public/gallery-cache';
    protected string $publicJsonDir  = '/volume1/web_gallery/public/gallery-json';
    ...
    public function show($id = null)
    {
        ...
        // ใช้ JSON cache เพื่อลด getimagesize; ถ้าไม่มีค่อยคำนวณ
        $cached = $this->tryReadAlbumJson($this->getAlbumJsonPath($albumIdShort));
        ...
        // เขียน JSON cache ต่ออัลบัม (เฉพาะใหม่หรือ refresh=1)
        try {
            $refresh  = ((int) ($this->request->getGet('refresh') ?? 0)) === 1;
            $jsonPath = $this->getAlbumJsonPath($albumIdShort);
            $existing = $this->tryReadAlbumJson($jsonPath);

            if ($this->shouldWriteAlbumJson($existing, count($images), $refresh)) {
                $payload = [
                    'id'     => $albumIdShort,
                    'store'  => $this->storeKey,
                    'subdir' => $subdir,
        ...
        return $this->respond([
            ...
            'count'  => count($images),
            'images' => $images,
            'json'   => ['path' => $this->getAlbumJsonPath($albumIdShort)],
        ]);
    }
