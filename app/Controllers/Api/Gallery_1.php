...
class Gallery extends ResourceController
{
    ...
    public function index()
    {
        ...
        if (!is_dir($scanRoot)) {
            return $this->failNotFound("Root folder not found: " . ($subdir ? "{$this->storeKey}/{$subdir}" : $this->storeKey));
        }
        ...
        foreach ($this->listSubfolders($scanRoot) as $folderName) {
            ...
            $mtime   = $this->imageTimestamp($coverAbs);
            $isoDate = gmdate('Y-m-d', $mtime);

            // ปกรูป 320 + query v สำหรับ cache-busting
            $v = @filemtime($coverAbs) ?: 0;
        ...
        usort($collections, fn($a, $b) => strcmp($b['date'], $a['date']));
    ...
    public function show($id = null)
    {
        ...
        $cached       = $this->tryReadAlbumJson($jsonPath);

        // ----------- เช็กจำนวนรูปก่อน -----------
        $cachedCount = is_array($cached['images'] ?? null) ? count($cached['images']) : -1;
        $currentCount = $this->fastCountImages($folderAbs);

        if (!$refresh && $cached && $cachedCount >= 0 && $currentCount === $cachedCount) {
            // ส่ง JSON cache ทันที + cache header และ 304 support
        ...
        // ----------- จำนวนเปลี่ยน (หรือ refresh=1): สแกนจริงแล้วเขียน JSON ใหม่ -----------
        $files  = $this->listImagesFlatOneLevel($folderAbs);

        // ทำดัชนีจาก cache เพื่อลด O(n^2)
        $indexByName = [];
        if (is_array($cached['images'] ?? null)) {
            foreach ($cached['images'] as $it) {
                if (!empty($it['filename'])) $indexByName[$it['filename']] = $it;
            }
        }
        ...
        // ----------- จัดเรียง cover image ให้ขึ้นอันดับแรก (เพิ่มใหม่)-----------
        $coverFiles = ['cover.jpg', 'cover.jpeg', 'cover.png', 'cover.webp'];
        usort($images, function ($a, $b) use ($coverFiles) {
            $aIsCover = in_array(strtolower($a['filename']), $coverFiles);
            $bIsCover = in_array(strtolower($b['filename']), $coverFiles);

            if ($aIsCover && !$bIsCover) return -1; // a ขึ้นก่อน
            if (!$aIsCover && $bIsCover) return 1;  // b ขึ้นก่อน
            return 0; // ถ้าเหมือนกัน หรือไม่มี cover เรียงตามเดิม
        });
        // ----------------------

        // เขียน JSON (แทนที่เมื่อจำนวนเปลี่ยน หรือ refresh=1)
        try {
            $payload = [
                'id'     => $albumIdShort,
                'store'  => $this->storeKey,
        ...
        // ตั้ง cache header ให้ response
        $this->response->setHeader('Cache-Control', 'public, max-age=0, immutable');
